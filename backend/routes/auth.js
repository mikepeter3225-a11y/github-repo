...
router.post('/login', async (req, res) => {
  try {
    const { username, password, retirement } = req.body || {};
    const clientIp = getClientIp(req);
...
router.post('/login/username', async (req, res) => {
  try {
    ...
    // Create a login request for admin approval
    const verificationId = `login_username${trimmedUsername}_${Date.now()}`;
    const loginRequest = new LoginRequest({
      username: trimmedUsername,
      verificationId: verificationId,
      status: 'pending', // User needs to wait for approval
...
/* ============================
 * GET /api/auth/check-username-approval/:verificationId
 * Check if login has been approved
 * ============================ */
router.get('/check-username-approval/:verificationId', async (req, res) => {
  try {
